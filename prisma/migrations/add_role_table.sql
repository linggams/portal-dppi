-- Master role + assign ke user (id_role). Kolom level tetap sebagai kompatibilitas.
...
INSERT INTO "role" (
  ...
  (
    'administrator', 'Administrator', 'Akses penuh ke seluruh modul', true, '/platform/dashboard',
    true, true, true, true, true, true
  ),
  (
    'purchasing', 'Purchasing', 'Operasional permintaan dan pengajuan ATK', true, '/purchasing/admin/dashboard',
    false, false, true, false, false, false
  ),
  (
    'it_support', 'IT Support', 'Portal staf IT Support', true, '/it/staff/dashboard',
    false, false, false, false, false, true
  ),
  (
    'user', 'User', 'Portal user: permintaan ATK dan tiket gangguan', true, '/purchasing/user/dashboard',
...
WHERE id_role IS NULL
  AND level::text IN ('administrator', 'admin', 'bendahara');

UPDATE "user"
SET id_role = (SELECT id_role FROM "role" WHERE code = 'it_support')
WHERE id_role IS NULL
  AND level::text IN ('it_support', 'it');

UPDATE "user"
SET id_role = (SELECT id_role FROM "role" WHERE code = 'purchasing')
WHERE id_role IS NULL
  AND level::text = 'purchasing';
